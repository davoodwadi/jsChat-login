...
const db = "chat";
const col = "chatUsers";
const MAX_TOKENS_PER_MONTH = 1000000; // 1 million tokens
...
export async function getUser(query) {
  ...
  try {
    ...
    const oneDoc = await collection.findOne(query);
...
export async function addUser(user) {
  try {
    const collection = getDB(db, col);
    if ((await collection.countDocuments({ username: user.username })) !== 0) {
...
export async function addSaveContainer(username, update) {
  try {
    ...
    if ((await collection.countDocuments({ username: username })) === 0) {
      ...
      const updateDoc = {
        $push: {
          sessions: { time: new Date(), saveContainer: update }, // Use $push to add to the array
        },
      };
      const result = await collection.updateOne(
        { username: username },
